  ...
  return (
    <div className='bg-neutral-900 border-2 border-neutral-800/90 py-2 shadow-xl rounded-lg grid max-h-72 overflow-y-auto'>
      {tracks[0].id.length > 0 && tracks?.map((song: TRACK, index: number) =>  (
        <button key={index} onClick={() => setIndex(index)} className='relative w-[340px] gap-3 text-start flex justify-between hover:bg-neutral-700/90 p-3'>
          ...
          {indexS === index && (
            <div className='absolute h-2 w-2 right-3 self-center bg-blue-500 rounded-full'></div>
          )}
        </button>
      ))}
